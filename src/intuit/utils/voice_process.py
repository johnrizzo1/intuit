def _audio_callback(self, indata, frames, time, status):
    ...
    if indata.size > 0:
        ...
        logger.debug(f"Speech detection: is_speech={is_speech}, volume={volume}, pitch={pitch}")
...
def start(self) -> bool:
    ...
    try:
        ...
        logger.debug(f"Voice process configuration: language={self.language}, slow={self.slow}, volume_threshold={self.volume_threshold}, speech_window={self.speech_window}")
...
def log_resource_usage(self):
    if self.process is not None and self.process.is_alive():
        ...
        logger.info(f"Voice process resource usage: CPU={cpu_usage}%, Memory={memory_usage}MB")
