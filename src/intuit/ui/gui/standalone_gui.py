...
try:
    ...
    VOICE_PROCESS_AVAILABLE = True
except ImportError:
    # When run as a standalone script or voice dependencies not available
    print(
        "Warning: Voice process manager not available. Voice functionality will be disabled."
    )
    VoiceProcessManager = None
    VOICE_PROCESS_AVAILABLE = False
...
class PulseEffect:
    ...
    def apply(self, painter: QPainter, rect: QRectF) -> None:
        ...
        gradient = QRadialGradient(
            center, radius * 1.2
        )  # Reduced radius multiplier (was 1.5)
        ...
        painter.drawEllipse(
            rect.adjusted(-radius / 3, -radius / 3, radius / 3, radius / 3)
        )  # Reduced adjustment (was radius/2)
        painter.restore()
...
class RippleEffect:
    ...
    def update(self, volume: float, pitch: float, dt: float) -> None:
        ...
        if volume > 0.1:
            self.spawn_interval = self.max_spawn_interval - volume * (
                self.max_spawn_interval - self.min_spawn_interval
            )
            self.spawn_timer += dt
                ...
                self.ripples.append(
                    {
                        "radius": 0.0,
                        "max_radius": 1.0
                        + volume * 0.5,  # Larger ripples for louder sounds
    ...
    def update(self, volume: float, pitch: float, dt: float) -> None:
        ...
        if (
            ...
            if self.base_intensity > 0.1:
                ...
                r = max(
                    0,
                    min(255, self.color.red() + random.randint(-color_var, color_var)),
                )
                g = max(
                    0,
                    min(
                        255, self.color.green() + random.randint(-color_var, color_var)
                    ),
                )
                b = max(
                    0,
                    min(255, self.color.blue() + random.randint(-color_var, color_var)),
                )
...
class HockeyPuckItem(QGraphicsEllipseItem):
    ...
    def paint(
        ...
        top_rect = QRectF(
            rect.x() + width * 0.05, rect.y() + height * 0.05, width * 0.9, height * 0.9
        )

        # Create a gradient for the top
        gradient = QLinearGradient(top_rect.topLeft(), top_rect.bottomRight())
...
class HockeyPuckView(QGraphicsView):
    ...
    def resizeEvent(self, event):
        ...
        self.puck_item.setPos(
            (self.width() - self.puck_size) / 2, (self.height() - self.puck_size) / 2
        )
    ...
    def mouseMoveEvent(self, event):
        ...
        if event.buttons() & Qt.LeftButton and self.drag_position is not None:
            ...
            # Move the window
            self.window().move(
                self.window().pos() + QPoint(int(delta.x()), int(delta.y()))
            )
            event.accept()
        else:
...
class IntuitGUI(QWidget):
    ...
    def __init__(self, config=None):
        ...
        if VOICE_PROCESS_AVAILABLE and VoiceProcessManager is not None:
            self.voice_manager = VoiceProcessManager(
                language=voice_language, slow=voice_slow
            )
            ...
            self.voice_active = False
            print("Voice functionality disabled due to missing dependencies")
    ...
    def reactToSpeech(self, volume: float, pitch: float):
        ...
        if self.speech_callback:
            self.speech_callback({"volume": volume, "pitch": pitch})
            ...
            if data_type == "metrics":
                ...
                if state == "start":
                    ...
                    if self.voice_active:
                        ...
                        QTimer.singleShot(
                            1000,
                            lambda: self.voice_manager.listen(
                                timeout=5.0, process=True
                            ),
                        )
        ...
        if self.voice_active:
            ...
            if self.voice_manager.start():
                ...
                self.voice_manager.speak(
                    "Voice interface activated. How can I help you?"
                )

                # Start a timer to begin listening after the welcome message
                QTimer.singleShot(
                    2000, lambda: self.voice_manager.listen(timeout=5.0, process=True)
                )
