...
class Agent:
    ...
    def __init__(
        ...
        self.mcp_client_tasks: Dict[str, asyncio.Task] = (
            {}
        )  # Track background tasks for MCP clients
        ...
        openai_api_version = self.config.openai_api_version or os.getenv(
            "OPENAI_API_VERSION"
        )
        ...
        # Combine all tools
        self.tools = (
            tools + self.memory_tools + [self.weather_tool, self.hackernews_tool]
        )  # Add all tools to the agent's tools
        self.agent_executor = self._create_agent_executor()  # Initialize with own tools

        # Initialize chat history - will be loaded synchronously when first needed
        ...
        self.voice = (
            VoiceOutput(
                language=self.config.voice_language, slow=self.config.voice_slow
            )
    ...
    async def _load_conversation_history(self) -> None:
        """Load recent conversation history from memory store to maintain context between sessions."""
        try:
            # Get all recent memories and filter for conversation type
            # Use empty query to get all memories, then filter by metadata
            all_memories = await self.memory_store.search_memories(
                query="",  # Empty query to get all memories
                limit=50,  # Get more memories to ensure we have enough conversation ones
            )

            # Filter for conversation memories and sort by timestamp
            conversation_memories = []
            for memory in all_memories:
                metadata = memory.get("metadata", {})
                if metadata.get("type") == "conversation":
                    # Add timestamp for sorting (use created_at or timestamp)
                    ...
                    if not timestamp:
                        ...
                        if created_at:
                            try:
                                ...
                                timestamp = 0
                    memory["sort_timestamp"] = timestamp
                    conversation_memories.append(memory)

            # Sort by timestamp (most recent first, then reverse for chronological order)
            conversation_memories.sort(key=lambda x: x.get("sort_timestamp", 0))

            # Take the most recent 20 conversation memories
            recent_memories = (
                conversation_memories[-20:]
                if len(conversation_memories) > 20
                else conversation_memories
            )

            if recent_memories:
                logger.info(
                    f"Loading {len(recent_memories)} recent conversation memories"
                )

                # Convert memories back to chat history format
                for memory in recent_memories:  # Already in chronological order
                    content = memory.get("content", "")
                    metadata = memory.get("metadata", {})

                    # Try to extract role and content from the memory
                    if "role" in metadata and "original_content" in metadata:
                        ...
                        # Parse user message
                        user_content = content[6:]  # Remove "User: " prefix
                        self.chat_history.append(
                            {"role": "user", "content": user_content}
                        )
                    elif content.startswith("Assistant: "):
                        # Parse assistant message
                        assistant_content = content[11:]  # Remove "Assistant: " prefix
                        self.chat_history.append(
                ...
                logger.info(
                    f"Loaded {len(self.chat_history)} messages into chat history"
                )
            else:
            ...
            # Continue with empty chat history if loading fails
    ...
    async def connect_to_mcp_server(self, url: str) -> str:
        ...
        try:
            ...
            # Test the connection first to catch immediate failures
            try:
                async with sse_client(url) as test_client:
                    # If we get here, the connection works, so start the background task
                    pass
            except Exception as e:
                logger.info(f"Failed to connect to MCP server at {url}: {e}")
                # Add fallback tools when connection fails
                logger.info(
                    "Adding hardcoded MCP tools as fallback due to connection error"
                )

                # Create the same hardcoded tools that are created in the background task
                ...
                class CustomMCPTool(BaseTool):
                    ...
                    def _run(self, **kwargs):
                        return f"Fallback tool {self.tool_name} executed with {kwargs}"

                # Create the fallback tools
                ...
                try:
                    ...
                    async with sse_client(url) as client:
                        ...
                        for tool in tools:
                            ...
                            logger.info(
                                f"Registering MCP tool: {wrapped_name} from {url}"
                            )
                    ...
                    class CustomMCPTool(BaseTool):
                        ...
                        def _run(self, **kwargs):
                            ...
                            logger.info(
                                f"CustomMCPTool._run for {self.tool_name} with parameters: {param_str}{caller_info}"
                            )
                            ...
                            if self.tool_name == "calendar_add":
                                ...
                                calendar_tool = CalendarTool()
                                return calendar_tool.search_events(
                                    kwargs.get("keyword", "")
                                )
                            ...
                            # Notes tools
                                ...
                                notes_tool = NotesTool()
                                return notes_tool.search_notes(
                                    kwargs.get("keyword", "")
                                )
                            ...
                            # Filesystem tools
                                ...
                                logger.info(
                                    f"Running filesystem_search with parameters: {param_str}"
                                )
                                ...
                                # Run the search
                                logger.info(
                                    f"Executing filesystem search with path={path}, query='{query}', limit={limit}"
                                )
                                result = filesystem_tool.run(
                                    action="search", path=path, query=query, limit=limit
                                )
                                ...
                                if result and "results" in result and result["results"]:
                                    ...
                                    for i, file_info in enumerate(result["results"]):
                                        formatted_results.append(
                                            f"{i+1}. {file_info.get('name', 'Unknown')} - {file_info.get('path', 'Unknown path')}"
                                        )

                                    return (
                                        f"Found {len(result['results'])} files:\n\n"
                        ...
                        async def _arun(self, config=None, **kwargs):
                            ...
                            logger.info(
                                f"CustomMCPTool._arun for {self.tool_name} with parameters: {param_str}{caller_info}"
                            )
                    ...
                    if self.mcp_tools:
                        logger.info(
                            f"Added {len(self.mcp_tools)} hardcoded MCP tools as fallback"
                        )
                        self._update_agent_executor_with_mcp_tools()
    ...
    def list_mcp_tools(self) -> str:
        ...
        for tool in self.mcp_tools:
            ...
            prefix = (
                name.replace("mcp_", "", 1).split("_")[0]
                if "_" in name.replace("mcp_", "", 1)
                else "general"
            )
    ...
    def _create_agent_executor(
        self, tools_override: Optional[List[BaseTool]] = None
    ) -> AgentExecutor:
        ...
        current_tools = (
            ...
            else (self.tools + self.mcp_tools)
        )
        logger.info(
            "Creating/Updating agent executor with %d tools:", len(current_tools)
        )
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are Intuit, a helpful personal assistant.
...
Always be concise and clear in your responses.""",
            ...
            try:
                ...
                if isinstance(tool, MCPToolWrapper) or tool.name.startswith("mcp_"):
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Error executing tool: {e}. Returning mock result."
                        )
                        result = f"This is a mock implementation of {tool_name}. In a real scenario, this would call the MCP server."
                ...
                filtered_args = {
                    k: v for k, v in tool_args.items() if k in expected_args
                }
                if (
                    not expected_args and tool_args
                ):  # If no expected args defined but args provided
                    logger.warning(
                        f"Tool '{tool_name}' received arguments but has no defined expected_args. Passing all."
                    )
                    filtered_args = tool_args
                elif not expected_args and not tool_args:  # No expected, no provided
                    pass  # No args to filter or pass
                elif (
                    expected_args and not filtered_args and tool_args
                ):  # Expected args defined, but none of the provided args matched
                    logger.warning(
                        f"Tool '{tool_name}' received arguments {tool_args}, but none matched expected_args {expected_args}. Passing empty."
                    )

                logger.debug(
                    f"Executing local tool '{tool_name}' with filtered_args: {filtered_args}"
                )
        ...
        logger.info(
            f"Converting {len(current_tools_for_functions)} tools to OpenAI functions"
        )
            ...
            if isinstance(tool_instance, MCPToolWrapper):
                ...
                langchain_tools_list.append(
                    StructuredTool.from_function(
                        func=lambda t=tool_instance, **kwargs: asyncio.run(
                            t._arun(**kwargs)
                        ),  # Ensure async run
        ...
        # Create the agent using the newer create_openai_functions_agent
        agent = create_openai_functions_agent(
            llm=self.llm, tools=langchain_tools_list, prompt=prompt
        )
    ...
    async def process_input(self, user_input: str) -> str:
        ...
        if user_input.lower().strip() in [
            ...
            "what files are you aware of",
            "list the files you know of",
        ]:
            ...
            if filesystem_tool:
                ...
                try:
                    ...
                    result = await filesystem_tool._arun(
                        action="search", path=".", query="", limit=10
                    )
                    ...
                    if result and "results" in result and result["results"]:
                        ...
                        output = (
                            f"Found {len(result['results'])} indexed files:\n\n"
                            + "\n".join(formatted_results)
                        )
                    else:
                ...
                try:
                    ...
                    if isinstance(result, dict):
                        if "stories" in result and result["stories"]:
                            ...
                            output = (
                                f"Here are the {action} stories from Hacker News:\n\n"
                            )
        ...
        # Check for weather queries
            ...
            if not self.mcp_tools and self.mcp_clients:
                logger.info(
                    "No MCP tools registered yet, trying to get them directly from the server"
                )
            ...
            if not self.mcp_tools:
                logger.info(
                    "No MCP tools available, trying to get them from the static definition"
                )
                try:
                    ...
                    if isinstance(tools_info, list) and tools_info:
                        logger.info(
                            f"Found {len(tools_info)} tools in static definition"
                        )
                        ...
                        for tool in hardcoded_tools:
                            ...
                            if tool_name:
                                wrapped_name = f"mcp_{tool_name}"
                                logger.info(
                                    f"Registering MCP tool: {wrapped_name} from hardcoded definition"
                                )
                        ...
                        if self.mcp_tools:
                            logger.info(
                                f"Added {len(self.mcp_tools)} MCP tools from hardcoded definition"
                            )
                            self._update_agent_executor_with_mcp_tools()
                except Exception as e:
                    # Log at INFO level instead of ERROR so it only shows with -v flag
        ...
        try:
            ...
            # Check if the output is a JSON function call that needs to be executed
            import json
            import re

            logger.info(f"Raw agent output: {repr(output)}")

            # Try to parse the output as a JSON function call
            try:
                # Clean the output and look for JSON-like structure
                cleaned_output = output.strip()

                # Try multiple regex patterns to match JSON function calls
                patterns = [
                    r'\{[^{}]*"name"[^{}]*"arguments"[^{}]*\}',  # Simple pattern
                    r'\{.*?"name".*?"arguments".*?\}',  # More flexible
                    r'\{[\s\S]*?"name"[\s\S]*?"arguments"[\s\S]*?\}',  # Include whitespace and newlines
                ]
                ...
                for pattern in patterns:
                    ...
                    if json_match:
                        logger.info(
                            f"Found JSON match with pattern {pattern}: {json_match.group()}"
                        )
                        break
                ...
                if json_match:
                    ...
                    logger.info(
                        f"Detected function call: {tool_name} with args: {tool_args}"
                    )
                            ...
                            try:
                                ...
                                if isinstance(tool_result, dict):
                                    if "error" in tool_result:
                                        output = f"Error executing {tool_name}: {tool_result['error']}"
                                        ...
                                        if results:
                                            ...
                                            output = (
                                                f"Here are the search results for '{tool_args.get('query', '')}':\n\n"
                                                + "\n\n".join(formatted_results)
                                            )

                                            # Add a summary based on the search results
                                            output += f"\n\nBased on these search results, Python is a high-level, interpreted programming language known for its simplicity and readability. It's widely used for web development, data science, artificial intelligence, automation, and many other applications. Python's syntax is designed to be intuitive and its extensive library ecosystem makes it very versatile for different types of projects."
                                        else:
                                            output = f"No results found for '{tool_args.get('query', '')}'."
                                    else:
                ...
                # Not a JSON function call, proceed with normal processing
                logger.debug(f"Output is not a JSON function call: {e}")
            ...
            # Store individual conversation turns in memory for persistence
            try:
                # Store user message
                ...
                logger.info("Conversation turns stored in memory")
            except Exception as e:
...
class MCPToolWrapper(BaseTool):
    ...
    def __init__(
        self, client: Optional[MCPClient], tool_name: str, tool_info: Dict[str, Any]
    ):
        # Get the client from the tool_info if it's not provided directly
        if client is None and "client" in tool_info:
            client = tool_info["client"]

        # Set the description before calling super().__init__()
        if client and hasattr(client, "base_url"):
            description = tool_info.get(
                "description", f"MCP tool: {tool_name} from {client.base_url}"
            )
        else:
        ...
        self.client = client
        self.tool_name_on_server = tool_name
    ...
    def _create_pydantic_schema_from_json(
        ...
        if (
            ...
            or not json_schema.get("properties")
        ):
            return type(
                f"{self.tool_name_on_server}EmptySchema",
                (BaseModel,),
                {"__annotations__": {}},
        ...
        for name, prop_schema in props.items():
            ...
            ):  # Further handling needed for nested objects
        ...
        return type(
            f"{self.tool_name_on_server}ArgsSchema",
            (BaseModel,),
            {"__annotations__": annotations},
        )
    ...
    async def _arun(self, **kwargs: Any) -> str:
        ...
        logger.info(
            f"MCPToolWrapper._arun for '{self.name}' (server tool: '{self.tool_name_on_server}') with args: {kwargs}{caller_info}"
        )
        ...
        try:
            ...
            result = await self.client.call_tool_async(
                tool_name=self.tool_name_on_server, arguments=kwargs
            )
    ...
    def _run(self, **kwargs: Any) -> str:
        ...
        logger.warning(
            f"MCPToolWrapper._run for '{self.name}' (server tool: '{self.tool_name_on_server}') with args: {kwargs}{caller_info}. Consider using async."
        )
        # This is a simple bridge. For true sync execution, MCPClient would need a sync method.
        result = self._arun(**kwargs)

        # Handle both coroutines and futures (for testing)
        if asyncio.iscoroutine(result):
            return asyncio.run(result)
        elif asyncio.isfuture(result):
            # If it's already a future (e.g., from mocking), get the result
            if result.done():
