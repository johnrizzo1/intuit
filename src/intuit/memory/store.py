...
class IntuitMemoryStore:
    ...
    def __init__(
        ...
        if os.getenv("REDIS_URL"):
            logger.warning(
                f"Redis URL found ({os.getenv('REDIS_URL')}), but Redis is not currently supported. Using in-memory storage instead."
            )
            ...
            logger.info(
                f"Using in-memory storage backend with persist directory: {persist_directory}"
            )
            self.store = InMemoryStore(
                index={
                    "dims": 1536,
        ...
        self.memory = create_memory_store_manager(
            model, namespace=(self.namespace,), store=self.store
        )
    ...
    async def search_memories(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for item in search_results:
                memory = {
                    "id": item.key,
                    "content": getattr(item.value, "content", ""),
    ...
    async def delete_memory(self, memory_id: str) -> bool:
        ...
        """
        try:
            result = await self.memory.adelete(memory_id)
            if result is False:
                logger.info(f"Memory with ID {memory_id} not found")
                return False
    ...
    async def update_memory(
        ...
        try:
            ...
            # Update memory
            existing_metadata = getattr(item.value, "metadata", {})
            new_metadata = metadata if metadata is not None else existing_metadata

            # Create updated memory data
            memory_data = {"content": content, "metadata": new_metadata}
