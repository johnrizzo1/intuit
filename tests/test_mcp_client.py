...
class TestMCPClient:
    ...
    async def test_connect_to_mcp_server(self, mock_sse_client, agent):
        ...
        mock_client.get_server_info.return_value = {"name": "Test Server", "version": "1.0.0"}
    ...
    async def test_mcp_tool_wrapper_arun_error(self):
        ...
        # Create a mock client
        mock_client = MagicMock()
        mock_client.call_tool_async = AsyncMock(side_effect=Exception("Tool execution failed"))
    ...
    def test_mcp_tool_wrapper_run(self):
        ...
        # Patch the _arun method
        with patch.object(wrapper, '_arun', return_value=asyncio.Future()) as mock_arun:
            mock_arun.return_value.set_result("Tool result")
